...
dbnames=${dbnames:0:-1}
echo "${DUMP_BIN} --host=${src_host} --db=${mongo_db} --collection=${dbinfo} --query="{db_name:{\$in:[${dbnames}]}}" --out=${BACKUP_DIR}"
${DUMP_BIN} --host=${src_host} --db=${mongo_db} --collection=${dbinfo} --query="{db_name:{\$in:[${dbnames}]}}" --out=${BACKUP_DIR}

# 2.2 restore db: featuredb
